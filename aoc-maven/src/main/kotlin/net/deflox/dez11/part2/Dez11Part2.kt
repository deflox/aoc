package org.example.net.deflox.dez11.part2
...
fun main() {
    ...
    val start = allNodes["svr"]!!
    val path = mutableListOf<String>()

    search(start, path)

    println(start.pathsToEnd.values.reduce { a, b -> a + b })

}

fun search(node: Node, currentPath: MutableList<String>) {
    ...
    if (!node.children.isEmpty()) {
        node.children.forEach { search(it, currentPath) }
    }
